...
import { BookStore } from '@stateMachines/machines'
...
const serverlessConfiguration: CustomServerless = {
  ...
  custom: {
    ...
    stepFunctionsLocal: {
      ...
      TaskResourceMapping: {
        ...BookStore.stateChain.getTaskToResourceMap(),
      },
    },
    dynamodb: {
      stages: ['offline'],
      ...
      seed: {
        standard: {
          sources: [
            {
              table: DdbTableBooks.stageQualifiedName,
              sources: ['./seed/books.json'],
            },
          ],
        },
      },
  ...
  resources: {
    Resources: {
      [DdbTableBooks.name]: DdbTableBooks.spec,
    },
  },
  functions: {
